...
// ===== QUICK PROBIV =====
async function quickProbiv() {
  const inputEl = document.getElementById('deceasedName');
  const fioRaw = inputEl.value.trim();
  if (!fioRaw) {
    showToast('Введіть ПІБ покійного для пробиву', 'error');
    return;
  }

  // Шукаємо дату народження (формат ДД.ММ.ГГГГ або ДД ММ ГГГГ)
  let dobMatch = fioRaw.match(/(\d{2})[^\d]?(\d{2})[^\d]?(\d{4})/);
  if (!dobMatch) {
    const m2 = fioRaw.match(/(\d{4})[^\d]?(\d{2})[^\d]?(\d{2})/); // YYYY-MM-DD
    if (m2) dobMatch = [m2[0], m2[3], m2[2], m2[1]];
  }
  ...
  if (dobMatch) {
    ...
    fioStr = fioRaw.replace(dobMatch[0], '').trim();
  } else {
    const userDob = prompt(`Введіть дату народження для "${fioStr}" у форматі ДД.ММ.ГГГГ (обов'язково для пошуку):`);
    if (!userDob) return;
    const m = userDob.match(/(\d{2})[^\d]?(\d{2})[^\d]?(\d{4})/);
    if (!m) {
      showToast('Невірний формат дати', 'error');
      return;
    }
    dob = `${m[1]} ${m[2]} ${m[3]}`;
    inputEl.value = `${fioStr} ${m[1]}.${m[2]}.${m[3]}`; // Зберігаємо дату в полі для зручності
  }

  const query = `${fioStr} ${dob}`;
  const btn = document.querySelector('button[onclick="quickProbiv()"]');
  const origContent = btn.innerHTML;
  btn.innerHTML = '⏳ Пробив...';
  btn.style.pointerEvents = 'none';
  btn.style.opacity = '0.7';

  try {
    const res = await fetch('/api/probiv/sauron', {
      method: 'POST',
    ...
    if (!res.ok || data.error) {
      showToast(data.error || 'Помилка пробиву', 'error');
    } else if (data.relatives && data.relatives.length > 0) {
      showToast(`Знайдено ${data.relatives.length} родичів!`, 'success');
      
      // Очищаємо порожній блок родича, якщо туди ще нічого не ввели
      const blocks = document.querySelectorAll('.relative-block');
      if (blocks.length === 1 && !blocks[0].querySelector('.rel-name').value && !blocks[0].querySelector('.rel-phone').value) {
        blocks[0].remove();
        relativeCount = 0;
      }
      ...
      data.relatives.forEach(rel => {
        addRelative();
        const newBlocks = document.querySelectorAll('.relative-block');
        const lastBlock = newBlocks[newBlocks.length - 1];
        if (lastBlock.querySelector('.rel-name')) lastBlock.querySelector('.rel-name').value = rel.name || rel.raw;
        if (lastBlock.querySelector('.rel-phone') && rel.phone) lastBlock.querySelector('.rel-phone').value = rel.phone;
      });
    } else {
      showToast('Связей не знайдено', 'info');
    }
  } catch(e) {
