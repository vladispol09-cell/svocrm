...
// ============ SAURON PROBIV (ФИО + Дата -> Родственники) ============
app.post('/api/probiv/sauron', requireAuth, async (req, res) => {
    const { query } = req.body;
    const API_TOKEN = "24EHqg5IID6HgvwUnjZcn3ySzC5Cx6nZX5nYAMaJVC06ujAQtvE2nX5tjxNb7KOm";

    try {
        // 1. Поиск в Sauron для получения ссылки на профиль
        const searchBody = new URLSearchParams();
        searchBody.append('token', API_TOKEN);
        searchBody.append('query', query);

        const searchRes = await fetch("https://sauron.info/api/v1/search/full", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: searchBody.toString()
        });
        const searchData = await searchRes.json();
        const redirectionUrl = searchData.result?.redirection;

        if (!redirectionUrl) {
            return res.json({ error: "Профиль не найден в базе. Проверьте ФИО и дату." });
        }

        // 2. Запрос HTML-страницы профиля для поиска блока 'Связь с лицом'
        const htmlRes = await fetch(redirectionUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                'Cookie': 'PHPSESSID=d63f57f796c9a37257edc28460bb5e8a; auth_token=NkZhw45pbedLbfRewbiYS0B5C71NK5HJ',
                'Referer': 'https://sauron.info/'
            }
        });
        const html = await htmlRes.text();

        // 3. Парсинг блока 'Связь с лицом' (Регулярное выражение вместо BeautifulSoup)
        const connections = [];
        const regex = /Связь с лицом[\s\S]*?class=["'][^"']*column--flex--result[^"']*["'][^>]*>([\s\S]*?)<\/div>/gi;
        let match;
        ...
        // 4. Поиск телефонов для найденных родственников (до 10 человек, чтобы не перегружать API)
        const relatives = [];
        for (const conn of uniqueConns.slice(0, 10)) {
            const tokens = conn.split(/\s+/);
            const fioParts = [];
            let dateRaw = "";
            for (const t of tokens) {
                if (fioParts.length < 3 && !/\d/.test(t)) fioParts.push(t);
                else dateRaw += " " + t;
            }

            let phone = null;
            if (fioParts.length >= 2) {
                const dMatch = dateRaw.match(/\d+/g);
                let day = "", month = "", year = "";
                if (dMatch && dMatch.length >= 3) {
                    day = dMatch[0].length === 4 ? dMatch[2] : dMatch[0];
                    month = dMatch[1];
                    year = dMatch[0].length === 4 ? dMatch[0] : dMatch[2];
                } else if (dMatch && dMatch.length === 1 && dMatch[0].length === 8) {
                    day = dMatch[0].slice(0, 2);
                    month = dMatch[0].slice(2, 4);
                    year = dMatch[0].slice(4, 8);
                }

                if (day && month && year) {
                    const reqData = new URLSearchParams();
                    reqData.append('token', API_TOKEN);
                    reqData.append('lastname', fioParts[0]);
                    reqData.append('firstname', fioParts[1]);
                    if (fioParts.length > 2) reqData.append('middlename', fioParts[2]);
                    reqData.append('day', day.padStart(2, '0'));
                    reqData.append('month', month.padStart(2, '0'));
                    reqData.append('year', year);

                    try {
                        const sumRes = await fetch("https://api.sauron.info/v1/query/get/person-summary", {
                            method: "POST",
                            headers: { "Content-Type": "application/x-www-form-urlencoded" },
                            body: reqData.toString()
                        });
                        const sumData = await sumRes.json();
                        phone = sumData.result?.response?.phone || null;
                    } catch(e) {}
                }
            }
            relatives.push({ raw: conn, name: fioParts.join(' ') || conn, phone });
        }
